...
parser.add_argument(
    '--output_dir', type=str, default='../dataset/uot_den_test_SENSE_1.02_0.8',
    help='Directory where to write output frames (If None, no output)')
parser.add_argument(
...
def test(cfg_data):
    ...
    for scene_id, sub_valset in enumerate(test_loader, 0):
        ...
        print(f"\nProcessing video {scene_id + 1}/20...")
            ...
            with torch.no_grad():
                ...
                if frame_signal == 'match':
                    ...
                    if kpts0.shape[0] > 0 and kpts1.shape[0] > 0:
                        ...
                        save_inflow_outflow_density(img[0].clone(), matched_results['scores'], matched_results['pre_points'],
                                                    osp.join(opt.output_dir, scene_name), scene_name, vi)
    ...
    np.save(os.path.join('./results',str(opt.test_intervals)+'_SENSE_cnt_1.5.py'),save_cnt_result.numpy())
...
def save_inflow_outflow_density(img_tensor, scores, pre_points, save_path, scene_id='', intervals=1):
    """Save density maps for inflow and outflow"""
    # Convert tensor to numpy array and prepare image
    img = img_tensor.cpu().numpy().transpose(1, 2, 0)  # Change from CxHxW to HxWxC
    img = (img * 255).astype(np.uint8)  # Scale from [0,1] to [0,255]
    
    h, w = img.shape[:2]
    
    inflow_map = np.zeros((h, w))
    outflow_map = np.zeros((h, w))
    
    if pre_points is not None and len(pre_points) > 0:
       
        
        # Convert points tensor to numpy array [num_frames, num_points, 2]
        if isinstance(pre_points, tuple):
            pre_tensor = pre_points[0]
        else:
            pre_tensor = pre_points
        if pre_tensor.device.type == 'mps':
             pre_tensor = pre_tensor.to('cpu')
        points_np = pre_tensor.detach().numpy()
        scores_np = scores.detach().cpu()
        scores_np = scores_np.detach().numpy()
        scores_np = np.nan_to_num(scores_np, nan=0.0, posinf=1.0, neginf=0.0)
        scores_np = np.clip(scores_np, 0.0, 1.0)  # Ensure scores are 0-1

        # Process inflow points (from second frame)
        # inflow_indices = np.where(scores[-1, :-1] > 0.4)[0]
        inflow_scores = scores[-1, :-1]
        if isinstance(inflow_scores, torch.Tensor) and inflow_scores.device.type != 'cpu':
             inflow_scores = inflow_scores.detach().cpu()
        inflow_indices = np.where(inflow_scores.numpy() > 0.4)[0]
        for idx in inflow_indices:
            if idx < len(points_np[1]):  # Ensure index exists
                x, y = points_np[idx, 2:4].astype(int)  # Extract x,y from points
                if 0 <= x < w and 0 <= y < h:
                    color_value = int(255 * scores_np[idx, -1])# Scale to 0-255
                    cv2.circle(outflow_map, (x, y), 15, (color_value, color_value, color_value), -1)
        
        # Process outflow points (from first frame)
       # outflow_scores = scores[:-1, -1]
        outflow_scores = scores[:-1, -1]
        if isinstance(outflow_scores, torch.Tensor) and outflow_scores.device.type != 'cpu':
            outflow_scores = outflow_scores.detach().cpu()
        outflow_indices = np.where(outflow_scores.numpy() > 0)[0]
        for idx in outflow_indices:
            if idx < len(points_np[0]):  # Ensure index exists
    ...
    # Apply Gaussian smoothing and save as before
    inflow_map = cv2.GaussianBlur(inflow_map, (31, 31), 8)
    outflow_map = cv2.GaussianBlur(outflow_map, (31, 31), 8)
    
    if inflow_map.max() > 0:
        inflow_map = (inflow_map / inflow_map.max())
    if outflow_map.max() > 0:
        outflow_map = (outflow_map / outflow_map.max())
    
    inflow_vis = (plt.cm.jet(inflow_map)[:, :, :3] * 255).astype(np.uint8)
    outflow_vis = (plt.cm.jet(outflow_map)[:, :, :3] * 255).astype(np.uint8)
    
    Path(save_path).mkdir(parents=True, exist_ok=True)
    base_name = f"{scene_id}_{intervals}_matches"
